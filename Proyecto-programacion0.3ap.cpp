...
int main()
{
	setlocale(LC_ALL, "");
	char nombre[10];
	char car,car1,car2,car3,car4,ca,c=0;
	...
	printf("\nTe consideras que estas en el camino espacioso?(s/n): ");
	scanf("%s",&car);
	if(car=='n')
	{
		Romanos();
	...
	if(contador>=3)
	{
		printf("\n<<<Felicidades, estas en el camino correcto>>>,\nrecuerda que una vez Cristo dijo:\n");
		Apoc_2114();
	}
	else
	{
		printf("\nEn realidad no somos quienes para juzgarte, pero si quieres, aún estas a tiempo\nVe este momento como uno clave para el cambio en tu vida,\nDios es un ser comprensivo y bueno.\n");
		printf("\nRecurda que Apocalipsis dice:\n");
		Apoc2014();
		system("cls");
		if(c==3)
		{
			printf("\nNoté que estas dispuesto a dejar todo por ser el hijo ideal, \nDios te esta esperando con manos abiertas...");
		}
	}
	return 0;
}
...
void Romanos()
{
	printf("\nQue si confiesas con tu boca que Jesús es el señor y crees que tu corazón Dios lo levantó de entre los muertos\nseras salvo. Porque con el corazón se cree para ser justificado, pero con la boca se confiesa para ser salvo.\n\n");
}

void Juan_3_18()
{
	printf("\nEl que cree en mi, que soy el Hijo de Dios, no sera condenado por Dios. Pero el que no cree ya ha sido condenado,\nprecisamente por no haber reido en el Hijo unico de Dios. ");

}

void Jeremias21_8()
{
	printf("\nY a este pueblo adviertele que asi dice el Senor: Pongo delante de ustedes el camino de la vida y el camino de la muerte.");
	
}

void Mateo7_13()
{
	printf("\nEntrad por la puerta estrecha; porque ancha es la puerta, y espacioso el camino que lleva a la perdicion,\n y muchos son los que entran por ella\n");

}

void Juan11_25_27()
{
	printf("\nLe dijo Jesus: Yo soy la resurreccion y la vida; el que cree en mi, aunque este muerto, vivira. Y todo aquel que vive y cree en mi, no morira eternamente. Crees esto?\nLe dijo: Si, Senor; yo he creido que tu eres el Cristo, el Hijo de Dios, que has venido al mundo.");
...
void Apoc_2114()
{
	printf("\nEntonces vi un nuevo cielo y una nueva tierra, porque la tierra, el mar y el cielo que conocemos desapareciero.\nY vi la ciudad santa, la nueva Jerusalén, descender del cielo, de donde estaba Dios.");
}
void Apoc2014()
{
